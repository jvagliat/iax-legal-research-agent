...
from iax_legal_lg_rag_research_agent.retrieval_graph.configuration import AgentConfiguration
from iax_legal_lg_rag_research_agent.retrieval_graph.researcher_graph.state import QueryState, ResearcherState
from iax_legal_lg_rag_research_agent.shared import retrieval
from iax_legal_lg_rag_research_agent.shared.utils import load_chat_model
...
async def retrieve_documents(
    ...
    """
    def _retrieve_with_retriever(query):
        with retrieval.make_retriever(config) as retriever:
            return retriever.invoke(query, config)
    
    response = await asyncio.to_thread(_retrieve_with_retriever, state.query)
